...
class FreshRSS_index_Controller extends Minz\ActionController {
	...
	public function normalAction() {
		...
		if (!FreshRSS_Auth::hasAccess() && !$allow_anonymous) {
			Minz\Request::forward(array('c' => 'auth', 'a' => 'login'));
			return;
		}
		...
		$this->view->rss_title = FreshRSS_Context::$name . ' | ' . Minz\View::title();
		$title = FreshRSS_Context::$name;
		if (FreshRSS_Context::$get_unread > 0) {
			$title = '(' . FreshRSS_Context::$get_unread . ') ' . $title;
		}
		Minz\View::prependTitle($title . ' · ');
	...
	public function globalAction() {
		...
		Minz\View::appendScript(Minz\Url::display('/scripts/extra.js?' . @filemtime(PUBLIC_PATH . '/scripts/extra.js')));
		Minz\View::appendScript(Minz\Url::display('/scripts/global_view.js?' . @filemtime(PUBLIC_PATH . '/scripts/global_view.js')));
	...
	private function updateContext() {
		...
		FreshRSS_Context::_get(Minz\Request::param('get', 'a'));

		FreshRSS_Context::$state = Minz\Request::param(
			'state', FreshRSS_Context::$user_conf->default_state
		);
		$state_forced_by_user = Minz\Request::param('state', false) !== false;
		...
		FreshRSS_Context::$order = Minz\Request::param(
			'order', FreshRSS_Context::$user_conf->sort_order
		);
		FreshRSS_Context::$number = intval(Minz\Request::param('nb', FreshRSS_Context::$user_conf->posts_per_page));
		...
		FreshRSS_Context::$first_id = Minz\Request::param('next', '');
		FreshRSS_Context::$sinceHours = intval(Minz\Request::param('hours', 0));
	}
	...
	public function aboutAction() {
		Minz\View::prependTitle(_t('index.about.title') . ' · ');
	}
	...
	public function tosAction() {
		...
		Minz\View::prependTitle(_t('index.tos.title') . ' · ');
	}
	...
	public function logsAction() {
		...
		Minz\View::prependTitle(_t('index.log.title') . ' · ');
		...
		//gestion pagination
		$page = Minz\Request::param('page', 1);
		$this->view->logsPaginator = new Minz\Paginator($logs);
