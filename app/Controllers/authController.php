...
class FreshRSS_auth_Controller extends FreshRSS_ActionController {
	...
	public function indexAction(): void {
		...
		if (Minz_Request::isPost()) {
			...
			if ($ok) {
				Minz_Request::good(_t('feedback.conf.updated'), [ 'c' => 'auth', 'a' => 'index' ], showNotification: FreshRSS_Context::userConf()->good_notification_timeout > 0);
			} else {
	...
	public function formLoginAction(): void {
		...
		if ($isPOST) {
			...
			if ($ok) {
				...
				Minz_Request::good(_t('feedback.auth.login.success'), $url, showNotification: FreshRSS_Context::userConf()->good_notification_timeout > 0);
			} else {
			...
			if ($ok) {
				...
				Minz_Request::good(_t('feedback.auth.login.success'), ['c' => 'index', 'a' => 'index'], showNotification: FreshRSS_Context::userConf()->good_notification_timeout > 0);
			} else {
	...
	public function logoutAction(): void {
		if (Minz_Request::isPost()) {
			...
			Minz_Session::regenerateID('FreshRSS');
			Minz_Request::good(_t('feedback.auth.logout.success'), [ 'c' => 'index', 'a' => 'index' ], showNotification: FreshRSS_Context::userConf()->good_notification_timeout > 0);
		} else {
			Minz_Error::error(403);
		}
	}
