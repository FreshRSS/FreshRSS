...
class FreshRSS_error_Controller extends FreshRSS_ActionController {
	...
	public function indexAction(): void {
		$code_int = Minz_Session::paramInt('error_code') ?: FreshRSS_HttpResponseCode::NOT_FOUND;
		/** @var array<string> */
		...
		switch ($code_int) {
		case FreshRSS_HttpResponseCode::OK :
			header('HTTP/1.1 200 OK');
			break;
		case FreshRSS_HttpResponseCode::BAD_REQUEST:
			header('HTTP/1.1 400 Bad Request');
			$this->view->code = 'Error 400 - Bad Request';
			$this->view->errorMessage = '';
			break;
		case FreshRSS_HttpResponseCode::FORBIDDEN :
			header('HTTP/1.1 403 Forbidden');
			$this->view->code = 'Error 403 - Forbidden';
			$this->view->errorMessage = _t('feedback.access.denied');
			break;
		case FreshRSS_HttpResponseCode::NOT_FOUND :
			header('HTTP/1.1 404 Not Found');
			$this->view->code = 'Error 404 - Not found';
			$this->view->errorMessage = _t('feedback.access.not_found');
			break;
		case FreshRSS_HttpResponseCode::METHOD_NOT_ALLOWED :
			header('HTTP/1.1 405 Method Not Allowed');
			$this->view->code = 'Error 405 - Method Not Allowed';
			$this->view->errorMessage = '';
			break;
		case FreshRSS_HttpResponseCode::SERVICE_UNAVAILABLE :
			header('HTTP/1.1 503 Service Unavailable');
			$this->view->code = 'Error 503 - Service Unavailable';
			$this->view->errorMessage = 'Error 503 - Service Unavailable';
			break;
		case FreshRSS_HttpResponseCode::INTERNAL_SERVER_ERROR :
		default:
