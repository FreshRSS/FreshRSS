...
class FreshRSS_category_Controller extends FreshRSS_ActionController {
	...
	public function createAction(): void {
		...
		if (Minz_Request::isPost()) {
			...
			if ($catDAO->addCategoryObject($cat)) {
				...
				Minz_Request::good(_t('feedback.sub.category.created', $cat->name()), $url_redirect, showNotification: FreshRSS_Context::userConf()->good_notification_timeout > 0);
			} else {
	...
	public function updateAction(): void {
		...
		if (Minz_Request::isPost()) {
			...
			if (false !== $categoryDAO->updateCategory($id, $values)) {
				Minz_Request::good(_t('feedback.sub.category.updated'), $url_redirect, showNotification: FreshRSS_Context::userConf()->good_notification_timeout > 0);
			} else {
				Minz_Request::bad(_t('feedback.sub.category.error'), $url_redirect);
			}
		}
	...
	public function deleteAction(): void {
		...
		if (Minz_Request::isPost()) {
			...
			Minz_Request::good(_t('feedback.sub.category.deleted'), $url_redirect, showNotification: FreshRSS_Context::userConf()->good_notification_timeout > 0);
		}
	...
	public function emptyAction(): void {
		...
		if (Minz_Request::isPost()) {
			...
			if ($feedDAO->deleteFeedByCategory($id, $muted, $errored)) {
				...
				Minz_Request::good(_t('feedback.sub.category.emptied'), $url_redirect, showNotification: FreshRSS_Context::userConf()->good_notification_timeout > 0);
			} else {
				Minz_Request::bad(_t('feedback.sub.category.error'), $url_redirect);
			}
		}
