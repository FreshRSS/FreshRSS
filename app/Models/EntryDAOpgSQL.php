...
class FreshRSS_EntryDAOpgSQL extends Minz_ModelPdo implements FreshRSS_Searchable {
	...
	public function addEntry($valuesTmp) {
		...
		$values = array(
			...
			$valuesTmp['is_read'] ? 't' : 'f',
			$valuesTmp['is_favorite'] ? 't' : 'f',
			$valuesTmp['id_feed'],
	...
	public function markRead($ids, $is_read = true) {
		if (is_array($ids)) {	//Many IDs at once (used by API)
			...
			$sql = 'UPDATE ' . $this->prefix . 'entry '
				 . 'SET is_read=? '
				 . 'WHERE id=? AND is_read=?';
			$values = array($is_read ? 't' : 'f', $ids, $is_read ? 'f' : 't');
			$stm = $this->bd->prepare($sql);
			if ($stm && $stm->execute($values)) {
				return $stm->rowCount();
			} else {
	...
	public function markReadCat($id, $idMax = 0) {
		...
		$sql = 'UPDATE ' . $this->prefix . 'entry e INNER JOIN ' . $this->prefix . 'feed f ON e.id_feed=f.id '
			 . 'SET e.is_read=true '
			 . 'WHERE f.category=? AND e.is_read=false AND e.id <= ?';
	...
	public function markReadFeed($id_feed, $idMax = 0) {
		...
		$sql = 'UPDATE ' . $this->prefix . 'entry '
			 . 'SET is_read=true '
			 . 'WHERE id_feed=? AND is_read=false AND id <= ?';
...
//		}
	...
	private function sqlListWhere($type = 'a', $id = '', $state = FreshRSS_Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filter = '', $date_min = 0) {
		...
		if ($state & FreshRSS_Entry::STATE_NOT_READ) {
			if (!($state & FreshRSS_Entry::STATE_READ)) {
				$where .= 'AND e1.is_read=false ';
			}
		}
		elseif ($state & FreshRSS_Entry::STATE_READ) {
			$where .= 'AND e1.is_read=true ';
		}
		if ($state & FreshRSS_Entry::STATE_FAVORITE) {
			if (!($state & FreshRSS_Entry::STATE_NOT_FAVORITE)) {
				$where .= 'AND e1.is_favorite=true ';
			}
		}
		elseif ($state & FreshRSS_Entry::STATE_NOT_FAVORITE) {
			$where .= 'AND e1.is_favorite=false ';
		}
	...
	public function countUnreadRead() {
		$sql = 'SELECT COUNT(e.id) AS count FROM ' . $this->prefix . 'entry e INNER JOIN ' . $this->prefix . 'feed f ON e.id_feed=f.id WHERE priority > 0'
		     . ' UNION SELECT COUNT(e.id) AS count FROM ' . $this->prefix . 'entry e INNER JOIN ' . $this->prefix . 'feed f ON e.id_feed=f.id WHERE priority > 0 AND is_read=false';
		$stm = $this->bd->prepare($sql);
		$stm->execute();
	...
	public function countNotRead($minPriority = null) {
		$sql = 'SELECT COUNT(e.id) AS count FROM ' . $this->prefix . 'entry e INNER JOIN ' . $this->prefix . 'feed f ON e.id_feed=f.id WHERE is_read=false';
		if ($minPriority !== null) {
	...
	public function countUnreadReadFavorites() {
		$sql = 'SELECT c FROM ('
		     .	'SELECT COUNT(id) AS c, 1 as o FROM ' . $this->prefix . 'entry WHERE is_favorite=true '
		     .	'UNION SELECT COUNT(id) AS c, 2 AS o FROM ' . $this->prefix . 'entry WHERE is_favorite=true AND is_read=false'
		     .	') u ORDER BY o';
		$stm = $this->bd->prepare($sql);
		$stm->execute();
		$res = $stm->fetchAll(PDO::FETCH_COLUMN, 0);
