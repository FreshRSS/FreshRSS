...
SQL;
	...
	protected function sqlListEntriesWhere(string $alias = '', int $state = FreshRSS_Entry::STATE_ALL, ?FreshRSS_BooleanSearch $filters = null,
		...
		return [$values, $search, $outerValues, $outerSearch];
	}
	...
	private function sqlListWhere(string $type = 'a', int $id = 0, int $state = FreshRSS_Entry::STATE_ALL, ?FreshRSS_BooleanSearch $filters = null,
		...
		[$searchValues, $search, $outerSearchValues, $outerSearch] = $this->sqlListEntriesWhere(alias: 'e.', state: $state, filters: $filters, id_min: $id_min, id_max: $id_max,
		...
		return [array_merge($values, $searchValues, $outerSearchValues), 'SELECT '
			. ($type === 'T' ? 'DISTINCT ' : '')
			. 'e.id'
			...
			. 'WHERE ' . $where . ($order === 'SHUF' ? ' AND (e.is_read=0 OR e.lastUserModified > UNIX_TIMESTAMP() - (24 * 60 * 60))' : '')
			. $search
			. ' ORDER BY '
