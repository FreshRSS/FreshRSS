...
function checkRequirements(string $dbType = ''): array {
	...
	$data = DATA_PATH && file_exists(DATA_PATH . '/index.html') && touch(DATA_PATH . '/index.html');	// is_writable() is not reliable for a folder on NFS
	// @phpstan-ignore-next-line
	$cache = CACHE_PATH && file_exists(CACHE_PATH . '/index.html') && touch(CACHE_PATH . '/index.html');
	// @phpstan-ignore-next-line
...
function getOwnerOfFile(string $file): array {
	...
	if (function_exists('posix_getpwuid') && function_exists('posix_getgrgid')) {
		$uid_name = @posix_getpwuid($stat_file['uid']);
		$gid_name = @posix_getgrgid($stat_file['gid']);
		$owner['fileowner'] = is_array($uid_name) ? $uid_name['name'] : _t('install.check.unknown_process_username');
