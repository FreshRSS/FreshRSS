...
foreach ($users as $user) {
	if (($user !== $system_conf->default_user) &&
		...
		Log::notice('FreshRSS skip inactive user ' . $user, ADMIN_LOG);
		if (defined('STDOUT')) {
			fwrite(STDOUT, 'FreshRSS skip inactive user ' . $user . "\n");	//Unbuffered
		}
		continue;
	}
	Log::notice('FreshRSS actualize ' . $user, ADMIN_LOG);
	if (defined('STDOUT')) {
	...
	if (!invalidateHttpCache()) {
		Log::warning('FreshRSS write access problem in ' . join_path(USERS_PATH, $user, 'log.txt'), ADMIN_LOG);
		if (defined('STDERR')) {
...
Log::notice('FreshRSS actualize done.', ADMIN_LOG);
if (defined('STDOUT')) {
	fwrite(STDOUT, 'Done.' . "\n");
