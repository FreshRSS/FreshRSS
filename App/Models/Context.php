...
class Context {
	...
	public static function init() {
		// Init configuration.
		self::$system_conf = Configuration::get('system');
		self::$user_conf = Configuration::get('user');

		//Legacy
		$oldEntries = (int)Context::$user_conf->param('old_entries', 0);
		$keepMin = (int)Context::$user_conf->param('keep_history_default', -5);
	...
	public static function _get($get) {
		...
		case 'f':
			...
			$feed = Context::$system_conf->allow_robots ? null : FreshRSS_CategoryDAO::findFeed(self::$categories, $id);
			if ($feed === null) {
				$feedDAO = Factory::createFeedDao();
				$feed = $feedDAO->searchById($id);
				if (!$feed) {
					throw new Context_Exception('Invalid feed: ' . $id);
				}
			}
		...
		case 'c':
			...
			if (!isset(self::$categories[$id])) {
				...
				if (!$cat) {
					throw new Context_Exception('Invalid category: ' . $id);
				}
			} else {
		...
		case 't':
			...
			if (!isset(self::$tags[$id])) {
				...
				if (!$tag) {
					throw new Context_Exception('Invalid tag: ' . $id);
				}
			} else {
				$tag = self::$tags[$id];
			}
		...
		case 'T':
			...
			break;
		default:
			throw new Context_Exception('Invalid getter: ' . $get);
		}
	...
	public static function isAutoRemoveAvailable() {
		...
		if (self::isStateEnabled(Entry::STATE_READ)) {
			return false;
		}
		if (!self::isStateEnabled(Entry::STATE_NOT_READ)) {
			return false;
		}
		return true;
	}
