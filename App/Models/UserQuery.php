...
class UserQuery {
	...
	/**
	 * @param array $query
	 * @param Searchable $feed_dao
	 * @param Searchable $category_dao
	 */
	public function __construct($query, Searchable $feed_dao = null, FreshRSS_Searchable $category_dao = null, FreshRSS_Searchable $tag_dao = null) {
		...
		$this->search = new BooleanSearch($query['search']);
	...
	private function parseCategory($id) {
		if (is_null($this->category_dao)) {
			throw new DAO_Exception('Category DAO is not loaded in UserQuery');
		}
	...
	private function parseFeed($id) {
		if (is_null($this->feed_dao)) {
			throw new DAO_Exception('Feed DAO is not loaded in UserQuery');
		}
	...
	private function parseTag($id) {
		if ($this->tag_dao == null) {
			throw new DAO_Exception('Tag DAO is not loaded in UserQuery');
		}
		$category = $this->category_dao->searchById($id);
		if ($tag) {
