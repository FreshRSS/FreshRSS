...
class ConfigurationSetter {
	...
	private function _default_view(&$data, $value) {
		...
		case 'all':
			...
			$data['default_state'] = (Entry::STATE_READ + FreshRSS_Entry::STATE_NOT_READ);
			break;
		case 'adaptive':
		case 'unread':
		default:
			$data['default_view'] = $value;
			$data['default_state'] = Entry::STATE_NOT_READ;
		}
	}
	...
	private function _ttl_default(&$data, $value) {
		$value = intval($value);
		$data['ttl_default'] = $value > Feed::TTL_DEFAULT ? $value : 3600;
	}
	...
	private function _allow_anonymous(&$data, $value) {
		$data['allow_anonymous'] = $this->handleBool($value) && Auth::accessNeedsAction();
	}
