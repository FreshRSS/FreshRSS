...
class TagDAO extends ModelPdo implements FreshRSS_Searchable {
	...
	public function createTagTable() {
		...
		try {
			...
			Log::warning('SQL ALTER GUID case sensitivity...');
			$databaseDAO = Factory::createDatabaseDAO();
			$databaseDAO->ensureCaseInsensitiveGuids();

			Log::warning('SQL CREATE TABLE tag...');
			$ok = $this->pdo->exec($SQL_CREATE_TABLE_TAGS) !== false;
		} catch (Exception $e) {
			Log::error('EntryDAO::createTagTable error: ' . $e->getMessage());
		}
		if ($hadTransaction) {
	...
	protected function autoUpdateDb($errorInfo) {
		if (isset($errorInfo[0])) {
			if ($errorInfo[0] === DatabaseDAO::ER_BAD_TABLE_ERROR || $errorInfo[0] === FreshRSS_DatabaseDAOPGSQL::UNDEFINED_TABLE) {
	...
	public function addTag($valuesTmp) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Log::error('SQL error addTag: ' . $info[2]);
			return false;
		}
	}
	...
	public function updateTag($id, $valuesTmp) {
		...
		if ($stm && $stm->execute($values)) {
			...
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Log::error('SQL error updateTag: ' . $info[2]);
			return false;
		}
	}
	...
	public function deleteTag($id) {
		...
		if ($stm && $stm->execute($values)) {
			...
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Log::error('SQL error deleteTag: ' . $info[2]);
			return false;
		}
	}
	...
	public function listTags($precounts = false) {
		...
		if ($stm !== false) {
			...
			Log::error('SQL error listTags: ' . $info[2]);
			return false;
		}
	}
	...
	public function count() {
		...
		if ($stm !== false) {
			...
			Log::error('SQL error TagDAO::count: ' . $info[2]);
			return false;
		}
	}
	...
	public function tagEntry($id_tag, $id_entry, $checked = true) {
		...
		if ($stm && $stm->execute($values)) {
			...
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Log::error('SQL error tagEntry: ' . $info[2]);
			return false;
		}
	}
	...
	public function getTagsForEntry($id_entry) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Log::error('SQL error getTagsForEntry: ' . $info[2]);
			return false;
		}
	}
	...
	public function getTagsForEntries($entries) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Log::error('SQL error getTagsForEntries: ' . $info[2]);
			return false;
		}
	}
