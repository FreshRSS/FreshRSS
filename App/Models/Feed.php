...
class Feed extends Model {
	...
	public function favicon() {
		return Url::display('/f.php?' . $this->hash());
	}
	...
	public function _url($value, $validate = true) {
		...
		if (empty($value)) {
			throw new BadUrl_Exception($value);
		}
		$this->url = $value;
	}
	...
	public function load($loadDetails = false, $noCache = false) {
		if ($this->url !== null) {
			if (CACHE_PATH === false) {
				...
				ExtensionManager::callHook('simplepie_before_init', $feed, $this);
				$mtime = $feed->init();

				if ((!$mtime) || $feed->error()) {
					$errorMessage = $feed->error();
					throw new Feed_Exception(
				...
				if (($mtime === true) || ($mtime > $this->lastUpdate) || $noCache) {
					//Log::debug('FreshRSS no cache ' . $mtime . ' > ' . $this->lastUpdate . ' for ' . $clean_url);
					$this->loadEntries($feed);	// et on charge les articles du flux
				} else {
					//Log::debug('FreshRSS use cache for ' . $clean_url);
					$this->entries = array();
				}
	...
	public function loadEntries($feed) {
		...
		if ($hasBadGuids != !$hasUniqueGuids) {
			...
			if ($hasBadGuids) {
				Log::warning('Feed has invalid GUIDs: ' . $this->url);
			} else {
				Log::warning('Feed has valid GUIDs again: ' . $this->url);
			}
			$feedDAO = Factory::createFeedDao();
	...
	public function _filtersAction($action, $filters) {
		...
		for ($k = count($filters) - 1; $k >= 0; $k --) {
			$filter = $filters[$k];
			if ($filter != '') {
				$filterAction = FilterAction::fromJSON(array(
						'search' => $filter,
	...
	public function pubSubHubbubPrepare() {
		$key = '';
		if (Context::$system_conf->base_url && $this->hubUrl && $this->selfUrl && @is_dir(PSHB_PATH)) {
			...
			if ($hubFile = @file_get_contents($hubFilename)) {
				...
				$key = sha1($path . Context::$system_conf->salt);
				$hubJson = array(
					'hub' => $this->hubUrl,
					'key' => $key,
				);
			...
			if (user_Controller::checkUsername($currentUser) && !file_exists($path . '/' . $currentUser . '.txt')) {
	...
	public function pubSubHubbubSubscribe($state) {
		$url = $this->selfUrl ? $this->selfUrl : $this->url;
		if (Context::$system_conf->base_url && $url) {
			$hubFilename = PSHB_PATH . '/feeds/' . base64url_encode($url) . '/!hub.json';
			$hubFile = @file_get_contents($hubFilename);
			if ($hubFile === false) {
			...
			$callbackUrl = checkUrl(Request::getBaseUrl() . '/api/pshb.php?k=' . $hubJson['key']);
			if ($callbackUrl == '') {
