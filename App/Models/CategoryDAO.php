...
class CategoryDAO extends ModelPdo implements FreshRSS_Searchable {
	...
	public function addCategory($valuesTmp) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Log::error('SQL error addCategory: ' . json_encode($info));
			return false;
		}
	}
	...
	public function updateCategory($id, $valuesTmp) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Log::error('SQL error updateCategory: ' . json_encode($info));
			return false;
		}
	}
	...
	public function deleteCategory($id) {
		...
		if ($stm && $stm->execute()) {
			...
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Log::error('SQL error deleteCategory: ' . json_encode($info));
			return false;
		}
	}
	...
	public function listCategories($prePopulateFeeds = true, $details = false) {
		if ($prePopulateFeeds) {
			...
			$stm = $this->pdo->prepare($sql);
			$values = [ ':priority_normal' => Feed::PRIORITY_NORMAL ];
			if ($stm && $stm->execute($values)) {
				...
				Log::error('SQL error listCategories: ' . json_encode($info));
				return false;
			}
		} else {
	...
	public function checkDefault() {
		...
		if ($def_cat == null) {
			$cat = new Category(_t('gen.short.default_category'));
			...
			if ($stm && $stm->execute($values)) {
				...
				$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
				Log::error('SQL error check default category: ' . json_encode($info));
				return false;
			}
		}
		return true;
