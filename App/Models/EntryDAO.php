...
class EntryDAO extends ModelPdo implements FreshRSS_Searchable {
	...
	protected function createEntryTempTable() {
		...
		try {
			...
			Log::warning('SQL CREATE TABLE entrytmp...');
...
SQL;
	...
	protected function autoUpdateDb($errorInfo) {
		if (isset($errorInfo[0])) {
			if ($errorInfo[0] === DatabaseDAO::ER_BAD_TABLE_ERROR) {
	...
	public function addEntry($valuesTmp, $useTmpTable = true) {
		...
		if ($this->addEntryPrepared && $this->addEntryPrepared->execute()) {
			...
			if ($this->autoUpdateDb($info)) {
				...
				Log::error('SQL error addEntry: ' . $info[0] . ': ' . $info[1] . ' ' . $info[2]
...
SQL;
	...
	public function updateEntry($valuesTmp) {
		...
		if ($this->updateEntryPrepared && $this->updateEntryPrepared->execute()) {
			...
			Log::error('SQL error updateEntry: ' . $info[0] . ': ' . $info[1] . ' ' . $info[2]
	...
	public function markFavorite($ids, $is_favorite = true) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Log::error('SQL error markFavorite: ' . $info[2]);
			return false;
		}
	}
	...
	protected function updateCacheUnreads($catId = false, $feedId = false) {
		...
		if ($stm && $stm->execute($values)) {
			...
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Log::error('SQL error updateCacheUnreads: ' . $info[2]);
			return false;
		}
	}
	...
	public function markRead($ids, $is_read = true) {
		...
		if (is_array($ids)) {	//Many IDs at once
			...
			if (!($stm && $stm->execute($values))) {
				$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
				Log::error('SQL error markRead: ' . $info[2]);
				return false;
			}
	...
	public function markReadEntries($idMax = 0, $onlyFavorites = false, $priorityMin = 0, $filters = null, $state = 0, $is_read = true) {
		...
		if (!($stm && $stm->execute(array_merge($values, $searchValues)))) {
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Log::error('SQL error markReadEntries: ' . $info[2]);
			return false;
		}
		$affected = $stm->rowCount();
		if (($affected > 0) && (!$this->updateCacheUnreads(false, false))) {
	...
	public function markReadCat($id, $idMax = 0, $filters = null, $state = 0, $is_read = true) {
		...
		if (!($stm && $stm->execute(array_merge($values, $searchValues)))) {
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Log::error('SQL error markReadCat: ' . $info[2]);
			return false;
		}
		$affected = $stm->rowCount();
	...
	public function markReadFeed($id_feed, $idMax = 0, $filters = null, $state = 0, $is_read = true) {
		...
		if (!($stm && $stm->execute(array_merge($values, $searchValues)))) {
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Log::error('SQL error markReadFeed: ' . $info[2] . ' with SQL: ' . $sql . $search);
			$this->pdo->rollBack();
			return false;
		}
		$affected = $stm->rowCount();
		...
		if ($affected > 0) {
			...
			if (!($stm && $stm->execute())) {
				$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
				Log::error('SQL error markReadFeed cache: ' . $info[2]);
				$this->pdo->rollBack();
				return false;
			}
		}
	...
	public function markReadTag($id = '', $idMax = 0, $filters = null, $state = 0, $is_read = true) {
		...
		if (!($stm && $stm->execute(array_merge($values, $searchValues)))) {
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Log::error('SQL error markReadTag: ' . $info[2]);
			return false;
		}
		$affected = $stm->rowCount();
		if (($affected > 0) && (!$this->updateCacheUnreads(false, false))) {
	...
	protected function sqlListEntriesWhere($alias = '', $filters = null, $state = Entry::STATE_ALL, $order = 'DESC', $firstId = '', $date_min = 0) {
		$search = ' ';
		$values = array();
		...
		switch ($order) {
			...
			case 'ASC':
				break;
			default:
				throw new EntriesGetter_Exception('Bad order in Entry->listByType: [' . $order . ']!');
		}
		if ($firstId !== '') {
	...
	private function sqlListWhere($type = 'a', $id = '', $state = Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filters = null, $date_min = 0) {
		if (!$state) {
		...
		case 'a':	//All PRIORITY_MAIN_STREAM
			$where .= 'f.priority > ' . Feed::PRIORITY_NORMAL . ' ';
			break;
		...
		case 'ST':	//Starred or tagged
			...
			break;
		default:
			throw new EntriesGetter_Exception('Bad type in Entry->listByType: [' . $type . ']!');
		}
	...
	public function listWhereRaw($type = 'a', $id = '', $state = Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filters = null, $date_min = 0) {
		...
		if ($stm && $stm->execute($values)) {
			...
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Log::error('SQL error listWhereRaw: ' . $info[2]);
			return false;
		}
	}

	public function listWhere($type = 'a', $id = '', $state = Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filters = null, $date_min = 0) {
	...
	public function listIdsWhere($type = 'a', $id = '', $state = Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filters = null) {	//For API
	...
	public function listHashForFeedGuids($id_feed, $guids) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Log::error('SQL error listHashForFeedGuids: ' . $info[0] . ': ' . $info[1] . ' ' . $info[2]
	...
	public function updateLastSeen($id_feed, $guids, $mtime = 0) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Log::error('SQL error updateLastSeen: ' . $info[0] . ': ' . $info[1] . ' ' . $info[2]
...
SQL;
		...
		$stm->bindValue(':priority_normal1', Feed::PRIORITY_NORMAL, PDO::PARAM_INT);
		$stm->bindValue(':priority_normal2', Feed::PRIORITY_NORMAL, PDO::PARAM_INT);
		$stm->execute();
		$res = $stm->fetchAll(PDO::FETCH_COLUMN, 0);
		rsort($res);
		$all = empty($res[0]) ? 0 : $res[0];
