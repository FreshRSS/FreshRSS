...
function initTranslate() {
	...
	Translate::reset($_SESSION['language']);
}
...
function saveStep1() {
	if (isset($_POST['freshrss-keep-install']) &&
		...
		$user_conf = Configuration::get('user');
...
function saveStep2() {
	if (!empty($_POST)) {
		...
		$ok = false;
		try {
			Session::_param('currentUser', $config_array['default_user']);
			$ok = initDb();
...
function saveStep3() {
	$user_default_config = Configuration::get('default_user');
	if (!empty($_POST)) {
		$system_default_config = Configuration::get('default_system');
		$_SESSION['title'] = $system_default_config->title;
		$_SESSION['auth_type'] = param('auth_type', 'form');
		if (user_Controller::checkUsername(param('default_user', ''))) {
		...
		Configuration::register('system', DATA_PATH . '/config.php', FRESHRSS_PATH . '/config.default.php');
		Context::$system_conf = Configuration::get('system');
		Translate::init($_SESSION['language']);

		Context::$system_conf->default_user = $_SESSION['default_user'];
...
function freshrss_already_installed() {
	...
	$system_conf = null;
	try {
		Configuration::register('system', $conf_path);
		$system_conf = Configuration::get('system');
	...
	$current_user = $system_conf->default_user;
	try {
		Configuration::register('user', join_path(USERS_PATH, $current_user, 'config.php'));
	} catch (FileNotExistException $e) {
		return false;
	}
...
?>
	...
	<form action="index.php?step=3" method="post">
		...
		<div class="form-group">
			...
			<div class="group-controls">
				<input type="text" id="default_user" name="default_user" autocomplete="username" required="required" size="16" pattern="<?= user_Controller::USERNAME_PATTERN ?>" value="<?= isset($_SESSION['default_user']) ? $_SESSION['default_user'] : '' ?>" placeholder="<?= httpAuthUser() == '' ? 'alice' : httpAuthUser() ?>" tabindex="3" />
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="group-controls">
				<div class="stick">
					...
					<a class="btn toggle-password" data-toggle="passwordPlain"><?= Themes::icon('key') ?></a>
				</div>
