 ...
 */
class importExport_Controller extends ActionController {
	/**
	...
	public function importFile($name, $path, $username = null) {
		...
		$this->catDAO = new CategoryDAO($username);
		...
		if ($type_file === 'zip' && extension_loaded('zip')) {
			...
			if (!is_resource($zip)) {
				...
				throw new Zip_Exception($zip);
			}
			while (($zipfile = zip_read($zip)) !== false) {
				if (!is_resource($zipfile)) {
					// zip_entry() can also return an error code!
					throw new Zip_Exception($zipfile);
				} else {
			...
			throw new ZipMissing_Exception();
	...
	public function importAction() {
		if (!Request::isPost()) {
			Request::forward(array('c' => 'importExport', 'a' => 'index'), true);
		}
		...
		if ($status_file !== 0) {
			Log::warning('File cannot be uploaded. Error code: ' . $status_file);
			Request::bad(_t('feedback.import_export.file_cannot_be_uploaded'),
		...
		try {
			...
			Request::bad(_t('feedback.import_export.no_zip_extension'),
				array('c' => 'importExport', 'a' => 'index'));
		} catch (Zip_Exception $ze) {
			Log::warning('ZIP archive cannot be imported. Error code: ' . $ze->zipErrorCode());
			Request::bad(_t('feedback.import_export.zip_error'),
				array('c' => 'importExport', 'a' => 'index'));
		}
		...
		foreach ($opml_elements as $elt) {
			if (isset($elt['xmlUrl'])) {
				...
				if (Context::$isCli && $nb_feeds >= $limits['max_feeds']) {
					Log::warning(_t('feedback.sub.feed.over_max',
									  $limits['max_feeds']));
					$ok = false;
					continue;
				}
				...
				if (!Context::$isCli && $limit_reached) {
					Log::warning(_t('feedback.sub.category.over_max',
	...
	private function addFeedOpml($feed_elt, $parent_cat) {
		...
		$url = Helper::htmlspecialchars_utf8($feed_elt['xmlUrl']);
		$name = Helper::htmlspecialchars_utf8($feed_elt['text']);
		$website = '';
		if (isset($feed_elt['htmlUrl'])) {
			$website = Helper::htmlspecialchars_utf8($feed_elt['htmlUrl']);
		}
		$description = '';
		if (isset($feed_elt['description'])) {
			$description = Helper::htmlspecialchars_utf8($feed_elt['description']);
		}
		...
		if ($error) {
			if (Context::$isCli) {
				...
				Log::warning('Error during OPML feed import from URL: ' . $url . ' in category ' . $cat->id());
			}
		}
	...
	private function addCategoryOpml($cat_elt, $parent_cat, $cat_limit_reached) {
		...
		$catName = Helper::htmlspecialchars_utf8($cat_elt['text']);
		$cat = new Category($catName);

		$error = true;
		if (Context::$isCli || !$cat_limit_reached) {
		...
		if ($error) {
			if (Context::$isCli) {
				...
				Log::warning('Error during OPML category import from URL: ' . $catName);
			}
		}
	...
	private function importJson($article_file, $starred = false) {
		...
		$mark_as_read = Context::$user_conf->mark_when['reception'] ? 1 : 0;
	 ...
	 * @return Feed if feed is in database at the end of the process,
	 *         else null.
	 */
	private function addFeedJson($origin) {
		...
		try {
			...
			$feed = new Feed($url);
			$feed->_category(CategoryDAO::DEFAULTCATEGORYID);
			$feed->_name($name);
			$feed->_website($website);
			if (!empty($origin['disable'])) {
				$feed->_ttl(-1 * Context::$user_conf->ttl_default);
			}
	...
	public function exportFile($export_opml = true, $export_starred = false, $export_labelled = false, $export_feeds = array(), $maxFeedEntries = 50, $username = null) {
		...
		if ($nb_files > 1) {
			...
			try {
				...
				throw new ZipMissing_Exception($e);
			}
		} elseif ($nb_files === 1) {
			// Only one file? Guess its type and export it.
			$filename = key($export_files);
			$type = self::guessFileType($filename);
			$this->sendFile('freshrss_' . Session::param('currentUser', '_') . '_' . $filename, $export_files[$filename], $type);
		}
		return $nb_files;
	}
	...
	public function exportAction() {
		...
		try {
			$nb_files = $this->exportFile(
					Request::param('export_opml', false),
					Request::param('export_starred', false),
					Request::param('export_labelled', false),
					Request::param('export_feeds', array())
				);
		} catch (ZipMissing_Exception $zme) {
			# Oops, there is no ZIP extension!
			Request::bad(_t('feedback.import_export.export_no_zip_extension'),
			                  array('c' => 'importExport', 'a' => 'index'));
		}
	 ...
	 * @param Feed $feed feed of which we want to get entries.
	...
	private function generateEntries($type, $feed = null, $maxFeedEntries = 50) {
		...
		if ($type === 's' || $type === 'S' || $type === 'T' || $type === 'ST') {
			...
			$this->view->entriesId = $this->entryDAO->listIdsWhere($type, '', Entry::STATE_ALL, 'ASC', -1);
			...
			$this->view->entriesRaw = $this->entryDAO->listWhereRaw($type, '', Entry::STATE_ALL, 'ASC', -1);
		} elseif ($type === 'f' && $feed != null) {
			$this->view->list_title = _t('sub.import_export.feed_list', $feed->name());
			$this->view->type = 'feed/' . $feed->id();
			$this->view->entriesId = $this->entryDAO->listIdsWhere($type, $feed->id(), Entry::STATE_ALL, 'ASC', $maxFeedEntries);
			$this->view->entryIdsTagNames = $tagDAO->getEntryIdsTagNames($this->view->entriesId);
			//The following is a streamable query, i.e. must be last
			$this->view->entriesRaw = $this->entryDAO->listWhereRaw($type, $feed->id(), Entry::STATE_ALL, 'ASC', $maxFeedEntries);
			$this->view->feed = $feed;
		}
	...
	private function sendZip($files) {
		...
		$day = date('Y-m-d');
		header('Content-Disposition: attachment; filename="freshrss_' . Session::param('currentUser', '_') . '_' . $day . '_export.zip"');
		readfile($zip_file);
		unlink($zip_file);
	}
