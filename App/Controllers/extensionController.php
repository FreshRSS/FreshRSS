 ...
 */
class extension_Controller extends ActionController {
	/**
	...
	protected function getAvailableExtensionList() {
		...
		if ($json === false) {
			Log::error('Could not fetch available extension from GitHub');
			return array();
		}
	...
	public function configureAction() {
		...
		$ext_name = urldecode(Request::param('e'));
	...
	public function enableAction() {
		...
		if (Request::isPost()) {
			...
			if (is_null($ext)) {
				Request::bad(_t('feedback.extensions.not_found', $ext_name),
				                  $url_redirect);
			}

			if ($ext->isEnabled()) {
				Request::bad(_t('feedback.extensions.already_enabled', $ext_name),
				                  $url_redirect);
			}
			...
			if ($ext->getType() === 'system' && Auth::hasAccess('admin')) {
				...
				$conf = Context::$user_conf;
			} else {
				Request::bad(_t('feedback.extensions.no_access', $ext_name),
				                  $url_redirect);
			}
			...
			if ($res === true) {
				...
				Request::good(_t('feedback.extensions.enable.ok', $ext_name),
				                   $url_redirect);
			} else {
				Log::warning('Can not enable extension ' . $ext_name . ': ' . $res);
				Request::bad(_t('feedback.extensions.enable.ko', $ext_name, _url('index', 'logs')),
				                  $url_redirect);
			}
		}
	...
	public function disableAction() {
		...
		if (Request::isPost()) {
			...
			if (!$ext->isEnabled()) {
				Request::bad(_t('feedback.extensions.not_enabled', $ext_name),
				                  $url_redirect);
			}
			...
			if ($res === true) {
				...
				Request::good(_t('feedback.extensions.disable.ok', $ext_name),
				                   $url_redirect);
			} else {
				Log::warning('Can not unable extension ' . $ext_name . ': ' . $res);
				Request::bad(_t('feedback.extensions.disable.ko', $ext_name, _url('index', 'logs')),
				                  $url_redirect);
			}
		}
	...
	public function removeAction() {
		...
		if (Request::isPost()) {
			...
			if ($res) {
				Request::good(_t('feedback.extensions.removed', $ext_name),
				                   $url_redirect);
			} else {
				Request::bad(_t('feedback.extensions.cannot_delete', $ext_name),
				                  $url_redirect);
			}
		}
