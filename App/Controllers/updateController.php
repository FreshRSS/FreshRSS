...
class update_Controller extends ActionController {
	...
	public function checkAction() {
		...
		if (file_exists(UPDATE_FILENAME)) {
			...
			Request::forward(array('c' => 'update'), true);
		...
		if (self::isGit()) {
			...
			$auto_update_url = Context::$system_conf->auto_update_url . '?v=' . FRESHRSS_VERSION;
	...
	public function applyAction() {
		if (!file_exists(UPDATE_FILENAME) || !is_writable(FRESHRSS_PATH) || Configuration::get('system')->disable_update) {
			Request::forward(array('c' => 'update'), true);
		}

		if (Request::param('post_conf', false)) {
			if (self::isGit()) {
			...
			ExtensionManager::callHook('post_update');

			if ($res === true) {
				@unlink(UPDATE_FILENAME);
				@file_put_contents(join_path(DATA_PATH, 'last_update.txt'), '');
				Request::good(_t('feedback.update.finished'));
			} else {
				Request::bad(_t('feedback.update.error', $res),
	...
	public function checkInstallAction() {
		View::prependTitle(_t('admin.check_install.title') . ' · ');
