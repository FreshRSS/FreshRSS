...
use Freshrss\Models\ConfigurationSetter;
use Minz\Configuration;
use Minz\ExtensionManager;
use Minz\FrontController;
use Minz\Request;
use Minz\Session;
...
class FreshRSS extends FrontController {
	...
	public function init() {
		...
		$configuration_setter = new ConfigurationSetter();
		...
		// created above.
		$current_user = Session::param('currentUser', '_');
		...
		if (Auth::hasAccess() || $system_conf->allow_anonymous) {
			$ext_list = Context::$user_conf->extensions_enabled;
			ExtensionManager::enableByList($ext_list);
		}

		self::checkEmailValidated();

		ExtensionManager::callHook('freshrss_init');
	}
	...
	private static function initAuth() {
		...
		if (Request::isPost()) {
			if (!is_referer_from_same_domain()) {
				...
				Translate::init('en');	//TODO: Better choice of fallback language
				Error::error(403, array('error' => array(
						_t('feedback.access.denied'),
						' [HTTP_REFERER=' . htmlspecialchars($http_referer, ENT_NOQUOTES, 'UTF-8') . ']'
					)));
			}
			if (!(Auth::isCsrfOk() ||
				(Request::controllerName() === 'auth' && Minz_Request::actionName() === 'login') ||
				(Request::controllerName() === 'user' && Minz_Request::actionName() === 'create' &&
	...
	public static function loadStylesAndScripts() {
		$theme = Themes::load(FreshRSS_Context::$user_conf->theme);
		if ($theme) {
			foreach(array_reverse($theme['files']) as $file) {
				...
				View::prependStyle(Minz_Url::display($url));
			}
		}
		//Use prepend to insert before extensions. Added in reverse order.
		if (Request::controllerName() !== 'index') {
			View::prependScript(Minz_Url::display('/scripts/extra.js?' . @filemtime(PUBLIC_PATH . '/scripts/extra.js')));
		}
		View::prependScript(Minz_Url::display('/scripts/main.js?' . @filemtime(PUBLIC_PATH . '/scripts/main.js')));
	}
	...
	private static function checkEmailValidated() {
		$email_not_verified = Auth::hasAccess() && FreshRSS_Context::$user_conf->email_validation_token !== '';
		$action_is_allowed = (
