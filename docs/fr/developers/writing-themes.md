...
nav_order: 140
redirect_from:
  - /en/developers/04_Frontend/02_Design.html
---
...
- an (optional) `icons` folder containing .svg (for icons), .ico (for
  favicons), and .png (for special cases) files to override existing icons
- a `thumbs` folder containing a file, `original.png` that will be used as
  the preview for the theme

`_frss.css` is normally added to the `metadata.json` file as a fallback for
missing aspects or as basement. The file is taken from the base theme.
