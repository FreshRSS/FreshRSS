...
* `Minz_HookType::FeedsListBeforeActualize` (`function($feedList) -> array | null`): will be executed before FreshRSS tries to update feeds. The list of feeds (array with (instance of FreshRSS\_Feed) to update will be passed as a parameter. Useful for modifying the order in which the feeds will be updated.  
