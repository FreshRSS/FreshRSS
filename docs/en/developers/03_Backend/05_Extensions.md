...
* `Minz_HookType::CategoryHtmlAttributes` (`function(FreshRSS_Category $category, string $attributes): string`): Allows extensions to add extra `data-*` HTML attributes to a category row in the feeds sidebar. Each enabled extension implementing this hook is called in turn with the category and the attributes string accumulated so far (built from any previously-called extension, initially an empty string), and must return the full attributes string to hand off to the next extension — typically `return $attributes . ' data-example="value"';`. This lets several extensions each contribute their own attributes to the same row. The returned string must be valid HTML attributes syntax (e.g. `data-example="value"`).
	Since the category's data ultimately originates from an untrusted remote feed, any value you interpolate into the attributes string must be escaped yourself (e.g. with `htmlspecialchars()`) to avoid a stored XSS vulnerability — the returned string is inserted into the page as-is.
...
* `Minz_HookType::FeedHtmlAttributes` (`function(FreshRSS_Feed $feed, string $attributes): string`): Allows extensions to add extra `data-*` HTML attributes to a feed row in the feeds sidebar. Each enabled extension implementing this hook is called in turn with the feed and the attributes string accumulated so far (built from any previously-called extension, initially an empty string), and must return the full attributes string to hand off to the next extension — typically `return $attributes . ' data-example="value"';`. This lets several extensions each contribute their own attributes to the same row. The returned string must be valid HTML attributes syntax (e.g. `data-example="value"`).
	Since the feed's data ultimately originates from an untrusted remote source, any value you interpolate into the attributes string must be escaped yourself (e.g. with `htmlspecialchars()`) to avoid a stored XSS vulnerability — the returned string is inserted into the page as-is.
