...
* `Minz_HookType::FeedTreeItemAttributes` (`function(array{item: FreshRSS_Feed|FreshRSS_Category, attributes: array<string,string>}) -> array`): allows extensions to add `data-*` attributes to feed and category items in the feed tree. The hook must return the received array after updating its `attributes` member.
