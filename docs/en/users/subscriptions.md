...
If "Purge Policy" has "By default" selected, then the [default purge policy](./configuration.md) is used and the other options are not displayed. Category options will override the default policy, but they will not override feed-specific options.
...
Articles can be automatically marked as read based on some search terms. See [filtering](./filtering-articles.md) for more information on how to create these filters.
